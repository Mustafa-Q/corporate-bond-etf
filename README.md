...
4. **Performance comparison** *(in progress)* — with vs. without liquidity considerations, across all characteristics.
...
output/                        # generated by the scripts above (git-ignored by default; see below)
...
Each script writes its outputs to `../output/` (paths are relative to `scripts/`).
