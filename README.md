...
data/
  ...
  fisd_issue_size.csv (gitignored) # per-bond FISD offering amount / amount outstanding, written by 00e (licensed field)
...
python 00e_parse_fisd_issue_size.py ../data/raw/fisd_lqd.csv                                # WRDS Mergent FISD export -> data/fisd_issue_size.csv (optional)
python 06_liquidity_proxy_validation.py  # Step 6 proxy validation (activity read from data/trace_activity_raw.csv; issue-size read if data/fisd_issue_size.csv exists)
...
- **Liquidity proxy** (ETF par holding size) is a stand-in for trading activity. Step 6 measured it against TRACE (Spearman ρ ≈ 0.3 with days traded / trade count) and Step 7 showed the par-based tilt in Steps 3–5 raises real trading activity by only a few thousandths of a score point where the TRACE-based tilt raises it 3–30× more. Read the original Step 3–5 frontier as a *size* tilt; the `--score trace` re-run is the tradability version. Its own caveats: 400 bonds (15.9% of benchmark weight, mostly 2026 new issues) carry an imputed peer-median score, and the TRACE window predates the holdings snapshot by ~7 months. True amount outstanding is still not in the repo.
